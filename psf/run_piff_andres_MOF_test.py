...
# flag values for coad comparison
D2D =  7.3055556e-5  #maximum separation in degrees
...
def parse_args():
    ...
    parser.add_argument('--coad_catalog', default='',
                        help='Coad catalog to match files')
    ...
    parser.add_argument('--star_flag', type=str, 
                        help='Select star galaxy separation method')
...
def read_findstars(star_file, cat_file, magzp,  coad_file,  star_flag):
    ...
    coad_data =  fitsio.read(coad_file)
    coad_data = coad_data.astype(coad_data.dtype.newbyteorder('='))
    dfcoad = pandas.DataFrame(coad_data)
    df['star_flag2'], df['ext_mof'], df['cm_t'] , df['d2d']  =  matchwithcoadd(df, dfcoad)
    ###
...
def matchwithcoadd(df1,  df2):
    from astropy.coordinates import SkyCoord
    from astropy import units as u
    import numpy as np
    
    c = SkyCoord(ra=list(df1['ra'])* u.degree, dec=list(df1['dec'])*u.degree)  
    catalog = SkyCoord(ra=list(df2['RA'])*u.degree, dec=list(df2['DEC'])*u.degree)
    ...
    if (len(matches)== len(df1)):
        ...
        df1['cm_t'] =  list(df2.loc[df2.index.isin(idx) ,  'CM_T'])
        df1['star_flag2'] = 0 * len(df1)
        #star =( ( (df1['ext_mof'] ==0)|(df1['ext_mof'] ==1) ) & (df1['d2d'] < D2D ))
        star =( (df1['ext_mof'] ==0)  & (df1['d2d'] < D2D ))
        df1.loc[star, 'star_flag2'] = 1
    return df1['star_flag2'],  df1['ext_mof'], df1['cm_t'], df1['d2d']   
def remove_bad_stars(df, ccdnum, tbdata,
                     mag_cut, nbright_stars, min_mag, max_mag,
                     use_tapebumps, tapebump_extra, reserve, fwhm,  star_flag):
    ...
    """
    #HERE MF
    use = df[star_flag] == 1 #boolean array, instead 1 True and instead 0 False#
    ...
    if min_mag > 0:
        use = use & (df['mag'] > min_mag) #change True for False if df['mag'] > min_mag
    ...
    #add column use to the pandas data frame 
    df['use'] = use
...
def plot_fs(df, filename,  star_flag):
...
def measure_star_shapes(df, image_file, noweight, wcs, use_ngmix, fwhm,  star_flag):
    """Measure shapes of the raw stellar images at each location.
    """
    print('Read in stars in file: ',image_file)
    #indexes where there is an star (row number, it start from 0)
    ...
    #set to 0 indexed rows of of column obs_flag 
...
def measure_piff_shapes(df, psf_file, image_file, noweight, wcs, use_ngmix, fwhm,  star_flag):
...
def measure_psfex_shapes(df, psfex_file, image_file, noweight, wcs, use_ngmix, fwhm,  star_flag):
...
def main():
    ...
    #Create the blacklist_file
    blacklist_file = '/data/catalogs/psf'
    if args.tag:
    ...
    # Work directoy is where catalogs are
    ...
    # File contents a list of exposures, if file is not flagged look
    # for exps if they are flagged, if not flagged use meds exposures
    if args.file != '':
    ...
    # Read YAML astrometry solution (i.e pixel to wcs). From the list
    # of exposure select only those that are in the pixmappy zone.
    ...
    for exp in exps:
        exp = int(exp)
        # choose the rows from exposures-ccds-Y3A1 file with the
        # selected exposures i.e exps. each cycle the variable is
        # reset.
        data = all_exp[all_exp['expnum'] == exp]
        #print(data)
    
        # TODO: trim to the listed bands above rather than here.
        # It makes sense, before assigment jump undesired bands (save memory) 
        ...
        for k, row in exp_df.iterrows():
            ...
            try:
                ...
                with fitsio.FITS(image_file, 'rw') as f:
                    ...
                    img -= bkg
                    f[1].write(img)
                    #Print_to_image(f[1].read())
                #print(img)
                 
                             
                if args.get_psfex:
                    psfex_file = wget(url_base, base_pah + '/psf/', wdir, root + '_psfexcat.psf')
                ...
                # Unfortunately, the desdm catalog in the cat
                # directory doesn't seem to be complete.  Looks like
                # it only has stars maybe?  Which is nominally fine,
                # but we're not sure if we trust their star selection.
                # So we run sextractor ourself. And remake the catalog
                
                # Here is where catalog will be saved
                cat_file = os.path.join(wdir, root + '_cat.fits')
                ...
                # Run findstars and create the star_file catalag
                ...
                #include some flags as ra and dec
                df = read_findstars(star_file, cat_file, magzp,  args.coad_catalog,  star_flag)
                ...
                # Cut the brighest magnitudes or other
                # exclusions/reservations. Besides, the star_flag
                # create a use flag (boolean) to avoid above features
                nstars, ntot = remove_bad_stars(
                        df, ccdnum, tbdata,
                        args.mag_cut, args.nbright_stars, args.min_mag, args.max_mag,
                        args.use_tapebumps, args.tapebump_extra, args.reserve, fits_fwhm,  star_flag)


                # Check if there are few or many stars.
                if nstars < FEW_STARS:
                ...
                print('   cf. header fwhm = ',fits_fwhm * 0.26) # ??? 0.26
                ...
                # Measure the shapes and sizes of the stars
                measure_star_shapes(df, image_file, args.noweight, wcs,
                                    args.use_ngmix, fwhm,  star_flag)
                ...
                if args.plot_fs:
                    fs_plot_file = os.path.join(wdir, root + '_fs.png')
                    plot_fs(df, fs_plot_file, star_flag)
